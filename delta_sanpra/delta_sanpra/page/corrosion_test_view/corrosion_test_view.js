frappe.pages['corrosion-test-view'].on_page_load = function(wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'corrosion-test-view',
		single_column: true
	});

	$(frappe.render_template("corrosion_test_view")).appendTo(page.body);
	...
	function get_filters() {
		const filters = [];
		const corrosion_test = ($id.val() || "").trim();
		...
		if (corrosion_test) {
			filters.push(["Corrosion Test", "inward_number", "like", `%${corrosion_test}%`]);
		}

		if (from_date) {
			filters.push(["Corrosion Test", "report_date", ">=", from_date]);
		}

		if (to_date) {
			filters.push(["Corrosion Test", "report_date", "<=", to_date]);
		}
		if (customer_name){
			filters.push(["Corrosion Test", "customer_name", "like", `%${customer_name}%`]);
		}
		if (challan_no){
			filters.push(["Corrosion Test", "challan_no", "like", `%${challan_no}%`]);
		}
		if (material_specification){
			filters.push(["Corrosion Test", "material_specification", "like", `%${material_specification}%`]);
		}
		if (document_id){
			filters.push(["Corrosion Test", "document_id", "like", `%${document_id}%`]);
		}
		if (workflow_state){
			filters.push(["Corrosion Test", "workflow_state", "like", `%${workflow_state}%`]);
		}
		if (test_method){
			filters.push(["Corrosion Test", "test_method", "like", `%${test_method}%`]);
		}
	...
	function render_rows(rows) {
		if (!rows.length) {
			$tbody.html(`<tr><td class="sample-inward-empty" colspan="13">${__("No Corrosion Test records found")}</td></tr>`);
			return;
		}
	...
	$tbody.on("click", ".sample-inward-open", function() {
		frappe.set_route("Form", "Corrosion Test", $(this).closest("tr").data("name"));
	});
