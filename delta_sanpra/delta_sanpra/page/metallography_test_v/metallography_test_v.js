frappe.pages['metallography-test-v'].on_page_load = function(wrapper) {
	...
	$(frappe.render_template("metallography_test_v")).appendTo(page.body);
	...
	function get_filters() {
		const filters = [];
		const metallography_test = ($id.val() || "").trim();
		...
		if (metallography_test) {
			filters.push(["Metallography Test", "inward_number", "like", `%${metallography_test}%`]);
		}

		if (from_date) {
			filters.push(["Metallography Test", "date", ">=", from_date]);
		}

		if (to_date) {
			filters.push(["Metallography Test", "date", "<=", to_date]);
		}
		if (customer_name){
			filters.push(["Metallography Test", "customer_name", "like", `%${customer_name}%`]);
		}
		if (challan_no){
			filters.push(["Metallography Test", "challan_no", "like", `%${challan_no}%`]);
		}
		if (material_specification){
			filters.push(["Metallography Test", "material_specification", "like", `%${material_specification}%`]);
		}
		if (document_id){
			filters.push(["Metallography Test", "document_id", "like", `%${document_id}%`]);
		}
		if (workflow_state){
			filters.push(["Metallography Test", "workflow_state", "like", `%${workflow_state}%`]);
		}
		if (test_method){
			filters.push(["Metallography Test", "test_method", "like", `%${test_method}%`]);
		}
	...
	function render_rows(rows) {
		if (!rows.length) {
			$tbody.html(`<tr><td class="sample-inward-empty" colspan="13">${__("No Metallography Test records found")}</td></tr>`);
			return;
		}
		...
		$tbody.html(rows.map(row => `
			<tr data-name="${value(row.name)}" class="${is_open_for_48_hours(row) ? "sample-inward-row-overdue" : ""}">
				...
				<td title="${value(row.date)}">${value(row.date)}</td>
	...
	$tbody.on("click", ".sample-inward-open", function() {
		frappe.set_route("Form", "Metallography Test", $(this).closest("tr").data("name"));
	});
