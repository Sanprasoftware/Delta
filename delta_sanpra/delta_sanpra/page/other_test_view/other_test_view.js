frappe.pages['other-test-view'].on_page_load = function(wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'other-test-view',
		single_column: true
	});
	$(frappe.render_template("other_test_view")).appendTo(page.body);
	...
	function get_filters() {
		...
		if (other_test) {
			filters.push(["Other Test", "mrn_no", "like", `%${other_test}%`]);
		}

		if (from_date) {
			filters.push(["Other Test", "report_date", ">=", from_date]);
		}

		if (to_date) {
			filters.push(["Other Test", "report_date", "<=", to_date]);
		}
		if (customer_name){
			filters.push(["Other Test", "customer_name", "like", `%${customer_name}%`]);
		}
		if (challan_no){
			filters.push(["Other Test", "challan_no", "like", `%${challan_no}%`]);
		}
		if (material_specification){
			filters.push(["Other Test", "material_specification", "like", `%${material_specification}%`]);
		}
		if (document_id){
			filters.push(["Other Test", "document_id", "like", `%${document_id}%`]);
		}
		if (workflow_state){
			filters.push(["Other Test", "workflow_state", "like", `%${workflow_state}%`]);
		}
		if (test_method){
			filters.push(["Other Test", "test_method", "like", `%${test_method}%`]);
		}
	...
	function render_rows(rows) {
		if (!rows.length) {
			$tbody.html(`<tr><td class="sample-inward-empty" colspan="11">${__("No Other Test records found")}</td></tr>`);
			return;
		}

		$tbody.html(rows.map(row => `
			<tr data-name="${value(row.name)}" class="${is_open_for_48_hours(row) ? "sample-inward-row-overdue" : ""}">
				<td class="sample-inward-open" title="${value(row.mrn_no)}">${value(row.mrn_no)}</td>
	...
	function load_rows() {
		$tbody.html(`<tr><td class="sample-inward-loading" colspan="11">${__("Loading...")}</td></tr>`);
	...
	$tbody.on("click", ".sample-inward-open", function() {
		frappe.set_route("Form", "Other Test", $(this).closest("tr").data("name"));
	});
