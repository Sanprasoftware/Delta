frappe.pages['physical-test-view'].on_page_load = function(wrapper) {
	...
	$(frappe.render_template("physical_test_view")).appendTo(page.body);
	...
	function get_filters() {
		const filters = [];
		const physical_test = ($id.val() || "").trim();
		...
		if (physical_test) {
			filters.push(["Physical Test", "inward_number", "like", `%${physical_test}%`]);
		}

		if (from_date) {
			filters.push(["Physical Test", "report_date", ">=", from_date]);
		}

		if (to_date) {
			filters.push(["Physical Test", "report_date", "<=", to_date]);
		}
		if (customer_name){
			filters.push(["Physical Test", "customer_name", "like", `%${customer_name}%`]);
		}
		if (challan_no){
			filters.push(["Physical Test", "challan_no", "like", `%${challan_no}%`]);
		}
		if (material_specification){
			filters.push(["Physical Test", "material_specification", "like", `%${material_specification}%`]);
		}
		if (document_id){
			filters.push(["Physical Test", "document_id", "like", `%${document_id}%`]);
		}
		if (workflow_state){
			filters.push(["Physical Test", "workflow_state", "like", `%${workflow_state}%`]);
		}
		if (test_method){
			filters.push(["Physical Test", "test_method", "like", `%${test_method}%`]);
		}
	...
	function render_rows(rows) {
		if (!rows.length) {
			$tbody.html(`<tr><td class="sample-inward-empty" colspan="13">${__("No Physical Test records found")}</td></tr>`);
			return;
		}
	...
	$tbody.on("click", ".sample-inward-open", function() {
		frappe.set_route("Form", "Physical Test", $(this).closest("tr").data("name"));
	});
